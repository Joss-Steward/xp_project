...
public class PDFAdventureWriter
{
	...
	/**
	 * If description is > 100 chars, split it into two
	 * strings based on nearest whitespace to middle
	 * @param contents
	 * @param description
	 */
	private void showAdventureDescription(PDPageContentStream contents, String description)
	{
		...
		try 
		{
			...
			{
				float descriptionWidth = DESCRIPTIONFONT.getStringWidth(description)/1000*fontSize;
				
				contents.beginText();
				contents.newLineAtOffset((pageSize.getHeight() - descriptionWidth)/2, 430);
			...
			else
			{
				...
				float desc1Width = DESCRIPTIONFONT.getStringWidth(desc1)/1000*fontSize;
				float desc2Width = DESCRIPTIONFONT.getStringWidth(desc2)/1000*fontSize;
