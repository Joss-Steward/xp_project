...
public class MovementMessagePackerTest
{
	...
	public void test()
	{
		...
		MovementMessage msg = (MovementMessage) packer.pack(new PlayerMovedReport(1,new Position(4,3)));
		assertEquals(1, msg.getPlayerID());
		assertEquals(new Position(4,3), msg.getPosition());
	};
	
	/**
	 * If the packer sees a report that a different player moved, it shouldn't pack anything (just returning null)
	...
	public void onlySendIfThisPlayer() throws AlreadyBoundException, NotBoundException
	{
		setup();
		MovementMessagePacker packer = new MovementMessagePacker();
		MovementMessage msg = (MovementMessage) packer.pack(new PlayerMovedReport(2,new Position(4,3)));
		assertNull(msg);
	}
