...
public class QuestLayerReader
{
		...
		if (allObjects != null)
		{
			...
			for (int i = 0; i < allObjects.getCount(); i++)
			{
				...
				if (type != null)
				{
					...
					{
					case "quest":
						getQuests().add(QuestLayerObjectReader.getQuest(property));
						break;
					case "task":
						getTasks().add(QuestLayerObjectReader.getTask(property));
						break;
					case "trigger":
						getTriggers().add(QuestLayerObjectReader
								.getTrigger(property));
						break;
					}
				}
	...
	public void buildRelationships()
	{
		...
		if (getTriggers() != null && getTasks() != null)
		{
			for (int i = 0; i < getTriggers().size(); i++)
			{
				...
				// look for trigger in player's tasks to link up there
				// trigger did not match task found in this map layer
				if (matched == false)
				{
					// TODO
				}
		...
		if (getTasks() != null && getQuests() != null)
		{
			...
			for (int i = 0; i < getTasks().size(); i++)
			{
				...
				// look for tasks in player's quests to link up there
				// task did not match quest found in this map layer
				if (matched == false)
				{
					// TODO
				}
	...
	/**
	 * @param questLayer the questLayer to set
	 */
